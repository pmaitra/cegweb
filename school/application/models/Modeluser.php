...
class Modeluser extends CI_Model{
    ...
    public function validate($user){
        ...
        $this->db->SELECT(array('id as userid','first_name','middle_name','last_name','profile_picture','passwordReset'));
...
public function checkOldPass($old_password)
{
 ...
 // ---===================== End =========================---//
    ...
    public function get_candidate_marks_log($application_id,$teacher_id)
    {
      
       $select_arr = array('candidate_marks_log.teacher_id','candidate_marks_log.marks','candidate_marks_log.marks_details','candidate_marks_log.created_date','candidate_marks_log.status_flag','candidate_marks_log.application_id','candidate_marks_log.application_id as user_id','users.first_name','users.middle_name','users.last_name','candidate_details.first_name','candidate_details.middle_name','candidate_details.last_name');
       
       
       $this->db->select($select_arr);
       $this->db->from('candidate_marks_log');
       $this->db->join('users', 'candidate_marks_log.teacher_id = users.id');
       $this->db->join('candidate_details', 'candidate_marks_log.candidate_id = candidate_details.id');
       
       $this->db->where('candidate_marks_log.application_id', $application_id);
       $this->db->where('candidate_marks_log.teacher_id', $teacher_id);
    ...
    public function get_candidate_marks_log_details($application_id)
    {
      
       $select_arr = array('candidate_marks_log.teacher_id','candidate_marks_log.marks',
           'candidate_marks_log.marks_details','candidate_marks_log.created_date',
           'candidate_marks_log.status_flag','candidate_marks_log.application_id',
           'candidate_marks_log.application_id as user_id','users.first_name',
           'users.middle_name','users.last_name','candidate_details.first_name as candidate_first_name',
           'candidate_details.middle_name as candidate_middle_name','candidate_details.last_name as candidate_last_name'
           );
       ...
       $this->db->from('candidate_marks_log');
       $this->db->join('users', 'candidate_marks_log.teacher_id = users.id');
       $this->db->join('candidate_courses_map', 'candidate_courses_map.id = candidate_marks_log.application_id');
       $this->db->join('candidate_details', 'candidate_courses_map.candidate_id = candidate_details.id');
        ...
        public function updateProPicture($user)
        {
            ...
            public function checkPassoutdate()
            {
               ...
               $select_arr = array('candidate_details.passout_date','candidate_details.id as candidate_id','candidate_details.first_name','candidate_details.middle_name',
                    ...
                    ,'candidate_details.professional_qualification','candidate_details.competitive_exams',
                    'candidate_details.competative_exam_score','candidate_details.academic_or_cocurricular_achievements',
                    'candidate_details.professional_experience','candidate_details.professional_achievements','candidate_courses_map.id as application_id',
                    'candidate_details.interview_score','candidate_details.admission_first_fee_flag',
                    'courses.name','courses.program_name','courses.id as course_id','courses.first_fee','courses.first_fee_end_date',
                   'courses.total_seat');
               $this->db->select($select_arr);
            ...
            public function get_all_survey_details($course_id = '')
            {
                $select_arr = array('course_survey_map.combination_id',
                    ...
                    'survey_combination_map.combination_name'
                    );
                $this->db->select($select_arr);
                $this->db->from('course_survey_map');
                $this->db->join('survey_combination', 'survey_combination.combination_id = course_survey_map.combination_id', 'left');
                ...
                $this->db->join('survey_head', 'course_survey_map.survey_id = survey_head.id');
                
                if(!empty($course_id))
                {
                    $this->db->where('course_survey_map.course_id', $course_id);
                }
                $this->db->where('survey_head.status', 1);
                $this->db->group_by('combination_id');
                $query = $this->db->get();
               $result_arr = $query->result_array();
            ...
            public function addUserSurvey($user)
            {
                ...
                $param['question_answer_id'] = $user['questions_answer_id'];
            ...
            public function display_course_list_for_user_request($user_id)
            {
                $select_arr = array('courses.drive_name ','courses.name ','courses.id as course_id');
                $this->db->select($select_arr);
                $this->db->from('candidate_details');
                $this->db->join('courses', 'courses.id = candidate_details.course_id'); 
                //$this->db->join('candidate_courses_map', 'candidate_courses_map.course_id = candidate_details.course_id'); 
            ...
            {
                $param['user_id'] = $this->session->userdata('loggedinuserid');
                $param['candidate_id'] = $user['candidate_id'];
                $param['application_id'] = $user['application_id'];
                $param['certificate_id'] = $user['certificate_id'];
                $param['course_id'] = $user['course_id'];
                $param['comments'] = $user['comments'];
                $param['request_type'] = $user['request_type'];
                $param['certificate_path'] = $user['certificate_path'];

                $param['created_date'] = date ('c');

                $this->db->insert('service_requests', $param);
            ...
            public function addAdminRequest($user)
            {
                $param['approval_status'] = $user['approval_status'];
                $param['certificate_path'] = $user['certificate_path'];
                
                $param['modified_date'] = date ('c');
                
                $this->db->where('id',  $user['request_id']);
                $this->db->update('service_requests', $param);
                $userid = $this->db->affected_rows();
            ...
            public function display_service_request_list($certificate_type='')
            {
                $select_arr = array('courses.name as drive_name','courses.id as course_id',
                    'service_requests.application_id','service_requests.created_date','service_requests.modified_date','courses.program_type'
                    ,'service_requests.approval_status as status','service_requests.certificate_path','certificates_details.name as certificate_name');
                $this->db->select($select_arr);
                $this->db->from('service_requests');
                $this->db->join('courses', 'courses.id = service_requests.course_id'); 
                $this->db->join('certificates_details', 'certificates_details.id = service_requests.certificate_id'); 
                $this->db->where('service_requests.user_id', $this->session->userdata('loggedinuserid'));
            ...
            public function admin_service_request_list($course_id='')
            {
                $select_arr = array('service_requests.id as request_id',
                    'service_requests.application_id','service_requests.created_date','service_requests.modified_date'
                    ,'service_requests.approval_status as status','candidate_details.first_name','candidate_details.middle_name',
                    'candidate_details.last_name','certificates_details.name as certificates_name'
                    ,'service_requests.certificate_path as certificate_path');
                $this->db->select($select_arr);
                $this->db->from('service_requests');
                //$this->db->join('courses', 'courses.id = service_requests.course_id'); 
                $this->db->join('candidate_details', 'candidate_details.id = service_requests.candidate_id'); 
                $this->db->join('certificates_details', 'certificates_details.id = service_requests.certificate_id'); 
                //$this->db->join('candidate_courses_map', 'candidate_courses_map.course_id = candidate_details.course_id'); 
                $this->db->where('service_requests.course_id ', $course_id);
            ...
            public function get_single_service_request_details($request_id)
            {
                $select_arr = array('service_requests.id as request_id','courses.name as course_name',
                    'service_requests.application_id','service_requests.created_date','service_requests.modified_date'
                    ,'service_requests.approval_status as status','candidate_details.first_name',
                    'candidate_details.middle_name','candidate_details.last_name','certificates_details.name as certificates_name',
                    'candidate_details.date_of_birth','candidate_details.passout_date');
                $this->db->select($select_arr);
                $this->db->from('service_requests');
                $this->db->join('courses', 'courses.id = service_requests.course_id'); 
                $this->db->join('candidate_details', 'candidate_details.id = service_requests.candidate_id'); 
                $this->db->join('certificates_details', 'certificates_details.id = service_requests.certificate_id'); 
                //$this->db->join('candidate_courses_map', 'candidate_courses_map.course_id = candidate_details.course_id'); 
                $this->db->where('service_requests.id ', $request_id);
            ...
            public function display_course_list_for_user_refund($user_id)
            {
                $select_arr = array('courses.drive_name ','courses.name ','courses.id as course_id','courses.first_fee ','courses.first_fee_refund_percentage ');
                $this->db->select($select_arr);
                $this->db->from('candidate_details');
                $this->db->join('courses', 'courses.id = candidate_details.course_id'); 
                //$this->db->join('candidate_courses_map', 'candidate_courses_map.course_id = candidate_details.course_id'); 
                $this->db->where('candidate_details.user_id', $user_id);
                $this->db->where('candidate_details.admission_first_fee_flag', 1);
                //$this->db->where('candidate_refund.candidate_id', $candidate_id);
            ...
            public function display_refund_list($candidate_id)
            {
                $select_arr = array('courses.name as drive_name','courses.id as course_id','courses.program_type','candidate_refund.amount','candidate_refund.status','candidate_refund.created_date');
                $this->db->select($select_arr);
                $this->db->from('candidate_refund');
                $this->db->join('courses', 'courses.id = candidate_refund.course_id'); 
                $this->db->where('candidate_refund.candidate_id', $candidate_id);
            ...
            public function addUserRefundRequest($user)
            {
                $param['invoice_id'] = 0;
                $param['course_id'] = $user['course_id'];
                $param['application_id'] = $user['application_id'];
                $param['candidate_id'] = $user['candidate_id'];
                $param['term_id'] = 0;
                $param['payment_method'] = "Online";
                $param['payment_type'] = "Netbanking";
                $param['payment_fee_type'] = "Booking Fee";
                $param['amount'] = $user['refund_amount'];
                $param['coupon_id'] = 0;
                $param['status'] = 0;

                $param['created_date'] = date ('c');

                $this->db->insert('candidate_refund', $param);
                $requests_id = $this->db->insert_id();
