...
class Home extends CI_Controller {
      ...
      public function wp_autologin()
      {
        ...
        $postdata = "log=" . $username . "&pwd=" . $password . "&wp-submit=Log%20In&redirect_to=" . $url . "wp-admin";
        $ch = curl_init();
      ...
      public function ldapcheck()
      {
        ...
        $ldapconn = ldap_connect($ldaphost,$port)
            ...
            $ldap_server = "10.0.2.4" ;
            $ldap_user   = "CN=web service account,OU=Service Accounts,DC=ad,DC=com" ;
            $ldap_pass   = "superadmin" ;

            $ad = ldap_connect($ldap_server,$port) or die("Could not connect to {$ldaphost}");
      ...
      public function loginSubmit()
      {
            ...
            else
            {
                ...
                if(!empty($result)){
                    ...
                    else if(!empty($postdata['ref_link']) && $postdata['ref_link'] == 'alumni')
                    {   
...
//pr($result);
                ...
                else
                {
                        if($resposeData['responseCode'] == 200)
                        {
                               ...
                               ';
                                ...
                                $html_message = general_email_template($user_name_display,$message,'A Big Thank You & Welcome!');
       ...
       public function ajaxAccademicSubmission()
      {
                ...
                $postdata['competitive_exams_check'] = $this->input->post('competitive_exams_check[]');
                $postdata['academic_achievement'] = $this->input->post('academic_achievement');
                //pr($postdata['academic_achievement']);
                $this->load->model('modelregistration');
